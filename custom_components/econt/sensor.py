"""Sensor platform for the Econt parcel tracker integration."""
...
from . import EcontConfigEntry
from .const import DOMAIN
from .coordinator import EcontCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: EcontConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Econt sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        EcontIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            EcontParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(EcontNextDeliverySensor(coordinator, entry))
    entities.append(EcontDeliveredParcelsSensor(coordinator, entry))
    entities.append(EcontLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class EcontIncomingParcelsSensor(
    CoordinatorEntity[EcontCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                EcontParcelSensor(self.coordinator, self._entry, barcode)
...
class EcontParcelSensor(CoordinatorEntity[EcontCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Econt parcel."""
    ...
    def __init__(
        self, coordinator: EcontCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class EcontNextDeliverySensor(
    ...
    def __init__(
        self, coordinator: EcontCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class EcontDeliveredParcelsSensor(
    CoordinatorEntity[EcontCoordinator], SensorEntity
):
    """Recently delivered tracked Econt parcels."""
...
class EcontLastUpdateSensor(
    CoordinatorEntity[EcontCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Econt was last polled successfully."""
