"""Services for the Econt parcel tracker integration.

`econt.track_parcel` / `econt.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Econt hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Econt is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Econt services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Econt tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Econt services (single-entry integration, so on unload)."""
