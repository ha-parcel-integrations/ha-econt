"""Econt parcel tracker custom component for Home Assistant."""
...
from .api import EcontApiClient
...
@dataclass
class EcontData:
    """Runtime data attached to the Econt config entry."""

    client: EcontApiClient
    coordinator: EcontCoordinator


type EcontConfigEntry = ConfigEntry[EcontData]


async def async_setup_entry(hass: HomeAssistant, entry: EcontConfigEntry) -> bool:
    """Set up Econt from a config entry."""
    # No auth: Econt tracking is public, so the HA-managed session is fine.
    client = EcontApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = EcontData(client=client, coordinator=coordinator)
...
async def _async_options_updated(
    hass: HomeAssistant, entry: EcontConfigEntry
) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: EcontConfigEntry) -> bool:
    """Unload the Econt config entry."""
