"""Calendar platform for the Econt parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Econt deliveries calendar from a config entry."""
    async_add_entities([EcontDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class EcontDeliveriesCalendar(CoordinatorEntity[EcontCoordinator], CalendarEntity):
    """A read-only calendar of expected Econt deliveries.
    ...
    def __init__(self, coordinator: EcontCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Econt parcel")
            description_parts = [
