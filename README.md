...
[![Downloads](https://img.shields.io/github/downloads/ha-parcel-integrations/ha-econt/total.svg)](https://github.com/ha-parcel-integrations/ha-econt/releases)
...
- **Hot (every 15 minutes)** — while any tracked parcel is out for delivery
  ...
  no window is known yet — in practice this tier doesn't engage for Econt
  yet: only individual tracking events, not a parcel's overall status, are
  currently confirmed to mean "out for delivery").
...
- A small, fixed per-hub offset is added on top, so not every Econt hub out
...
This is an independent, community-built project. It is not affiliated with, endorsed by, sponsored by, or supported by Econt, Home Assistant, or any other third party referenced in this project. Please don't contact Econt for support with this integration.
...
This integration may rely on public, unofficial, or undocumented carrier interfaces, accessed with your own account or API key where required. These may change or be withdrawn without notice and may be subject to Econt's terms. Data is sent only to Econt's own services or those of its group; this project operates no servers of its own. You are responsible for ensuring that your use complies with applicable law and those terms. Use is at your own risk; see the [licence](LICENSE) for warranty limitations.

This integration uses the same public tracking endpoint as the Econt consumer website.
