"""Tests for Econt sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.econt.const import ParcelStatus
from custom_components.econt.sensor import (
    EcontDeliveredParcelsSensor,
    EcontIncomingParcelsSensor,
    EcontLastUpdateSensor,
    EcontNextDeliverySensor,
    EcontParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = EcontIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = EcontParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = EcontParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = EcontNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = EcontNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = EcontDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = EcontLastUpdateSensor(coordinator, _entry())
